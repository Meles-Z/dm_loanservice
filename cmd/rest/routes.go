...
func initRoutes(ctx context.Context, e ie.Endpoints) *mux.Router {
	...
	accountRoutes.Handle("/{id:[0-9a-fA-F-]+}", ithttp.MakeAccountUpdateHandler((e.AccountUpdate))).Methods(http.MethodPut)
	...
	accountRoutes.Handle("/{id:[0-9a-fA-F-]+}/servicing-restrictions", ithttp.MakeServiceRestrictionReadByAccountHandler((e.ServiceRestrictionReadByAccount))).Methods(http.MethodGet)
	...
	// account audit log
	accountAuditLogRoutes := r.PathPrefix("/api/v1/account-audit-log").Subrouter()
	accountAuditLogRoutes.Handle("/{id:[0-9a-fA-F-]+}", ithttp.MakeAccountAuditLogReadHandler((e.AccountAuditLogRead))).Methods(http.MethodGet)
	// late fee rule
